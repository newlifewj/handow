...
// This API is only called by SHM POST: /api/handow/handowstatus routes, set the native running status.
const setRunningStatus = (running, pid) => {
    nativeRunPid = pid;
    isRunning = running;
};
 ...
 * 
 * In order to resolve this defect (make SHMUI knowing if native handow is running or not), the native-run will pass its running status
 * to shm-run process if shm installed and launched. So that SHMUI will know handow running state even it is from native-run.
 * And SHMUI also need to distinguish "native" or "shm" running, so the native-run add additional field - "pid", "pid" existing means 
 * native-running.
 * More important, SHM can stop native process by kill it by specify its pid.
 * 
 ...
 * the tree-burning for native-running. But SHMUI know native-run is running or not, and admin can stop the native-running from SHMUI
 */
