...
const transformStoryObj = (name, storyObj) => {
    ...
    for ( const gp of gParams ) {
        ...
        for ( const ga of gActs ) {
            _gActs.push({ label: instLabel(`<b>Act:</b> ${ga.label}`, ga.args, gp), status: "ready" });
        }

        const _gFacts = [];
        for ( const gf of gFacts ) {
            _gFacts.push({ label: instLabel(`<b>Fact:</b>${gf.label}`, gf.args, gp), status: "ready" });
        }
        ...
        for ( const when of storyObj.whens ) {
            ...
            for ( const wp of wParams ) {
                ...
                for ( const wa of wActs ) {
                    _wActs.push({ label: instLabel(`<b>Act:</b> ${wa.label}`, wa.args, { ...gp, ...wp }), status: "ready" });
                }

                const _wFacts = [];
                for ( const wf of wFacts ) {
                    _wFacts.push({ label: instLabel(`<b>Fact:</b>${wf.label}`, wf.args, { ...gp, ...wp }), status: "ready" });
                }
