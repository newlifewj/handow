...
const parseStepLiteral = (line) => {
    ...
    while ( result ) {
        ...
        const argar = result[0].trim().slice(1, -1).trim().split(":");

        let realArg = argar[1].trim();
        ...
        stepObj["args"][argar[0].trim()] = realArg;
        line = line.replace(result[0], ` <${argar[0].trim()}>`);
        result = regexp.exec(line);
    }

    // Remove the temporary "<>" mark, replaced with "{}"
    if ( stepObj["args"] ) {
        for ( let arg in stepObj["args"] ) {
        ...
        if ( i === 0 && block[i].startsWith("@skip:") ) {
            ...
            */
            ...
            if ( _param.startsWith("{") && _param.endsWith("}") ) {
                ...
                const _paramArray = _param.replace(/} ?,/, "}},").split("},");
