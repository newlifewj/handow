...
// const injectGlobalParams = require('./globalParams');
...
const runStage = async ( stage ) => {
    ...
    for ( let suiteName of stage.stories ) {
        ...
        const _suite = await parseStory( path.join(config._rootPath, config.storiesPath, suiteName) );
        suites.push(_suite);
        storyStatus.push({
            story: suiteName,
            status: "pending"
...
// -------------
     ...
     * 2, or emit "ALL_STAGES_FINISHED" if last stage finished.
     */
    eventBus.on('SUITE_FINISHED', async ( _suite ) => {
        ...
        if ( totalStageSuites > 0 ) {
            ...
            await runStage( plan.stages[stageIdx] );
            stageIdx = stageIdx + 1;
        } else {
            // console.log("The plan is finished");
            eventBus.emit('ALL_STAGES_FINISHED');
