...
let planBrowser;            // The browser instance in plan scope
    ...
    */
    if ( stage.newBrowserSession && planBrowser ) {
        try {
            ...
            sleep(1000);
            planBrowser = await launchBrowser({ headless: config.headless });

        } catch (err) {
            // ToDo: call breakRunning()
        }
    }
    ...
    */
    for (let i = 0; i < workers; i++) {
        if (suites[i]) {
            ...
            sleep(300);     // THINKING: really need sleep a while?
        }
    }
...
module.exports = async ( _plan, _pTree ) => {
    ...
    config = deepExtend( planConfig.get(), _plan.config );   // Use config of the plan to override the inherited config properties
    ...
    /*
        Launch browser instance if the 'browserSessionScope' is not per-story.
        When 'browserSessionScope' is configuraed as 'story', new browser instance will be opened and closed for each story in the suite runner
    */
    ...
    if ( totalStageSuites > 0 ) {
        ...
        */
        if ( waitingSuites.length > 0 ) {
            ...
            /* Put a story to runn after a stories finished, at least one worker is idle at this momnet */
            const _suite = waitingSuites.pop();
            suiteRunr( _suite, stageParams, planBrowser );
        ...
        /*
            ...
            1, Close all opened pages and the browser - if browser is plan-scoped.
            2, Output to console for native running
        */
        
        /* For the plan scoped browser, here close all opened pages and then close the browser */
        if (planBrowser) {
            const openedPages = await planBrowser.pages();
            await Promise.all(openedPages.map( (page) => page.close() ));
            await planBrowser.close();
