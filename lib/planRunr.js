...
// -------------
    ...
    // copy the plan tree json file to record, which is used by SHM report render
    try {
        if (plan.path.endsWith('.plan.json')) {
        fs.copyFileSync( plan.path.replace(".plan.json", ".tree.json"), path.join(config._rootPath, config.reportPath, 'plan.tree.json') );
    } else if (plan.path.endsWith('.feature')) {
        fs.copyFileSync( plan.path.replace(".feature", ".tree.json"), path.join(config._rootPath, config.reportPath, 'plan.tree.json') );
    }
    } catch (e) {
        //
    }
