...
const parseProbe = (key, value) => {
    ...
    */
        ...
        if ( key === 'selector' ) {                         // covert to "*[h4w=probe-name]"
            _v = value.replace(`@${config.htmlProbe}`, '');
            _v = `*[${config.htmlProbe}=${_v}]`;

        } else if ( key.trim() === 'xpath' ) {              // convert to "//*[@h4w='probe-name']"
            _v = value.replace(`@${config.htmlProbe}`, '');
            _v = `//*[@${config.htmlProbe}='${_v}']`;
        } else {
            _v = value;     // go through if it isn't a 'selector' or 'xpath' 
        }
    /*
        ...
        */
            ...
            if ( args.length === 1 ) {
                if ( !isNth(args[0]) && key === 'xpath') {
                    ...
                    /* "probe-name@h4w(5)" to css selector *[h4w=prifile-name]:nth-child(5), the nth number starts from 1 */
