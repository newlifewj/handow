...
module.exports = {
        ...
        record.stages[record.stages.length - 1]['stories'].map( (story) => {
            if ( story.story == suite.story || `${story.story}.feature` == suite.story ) {
                ...
                for ( const params of suite.given.parameters ) {
                    const _whens = [];
                    if (!suite.whens) { suite.whens = []; }  // For stories with only one Given scenario
        ...
        record.stages[record.stages.length - 1]['stories'].map( (story) => {
            ...
            if ( `${story.story}` === `${suite.story}` || `${story.story}.feature` === `${suite.story}` ) {
                ...
                if ( 'given' === phase && step.type === 'act' ) {
                    ...
                    console.log(`Cannot record the step data - [${phase}, ${step.type }]`);
                }
    ...
    save: async (tree) => {
        ...
        /*
            config.localRender===ture: Generate the static html report
            config.localRender===ture && config.localAutoRender===ture: Open local browser render the generated static html report 
        */
