...
const refreshSuitesStatus = ( suites ) => {
        ...
        const _ss = _.chunk( suites, planConfig.get().showSuitesPerLine );
        sLines = _ss.length;
...
module.exports = {
    ...
    runSuites: ( suites, stage ) => {
        if ( planConfig.get().consoleOutput !== 'step' && planConfig.get().consoleOutput !== 'none' ) {
    ...
    cliHelp: () => {
        ...
        tmnl("\u0009\u0009\u0009\u0009").green("--parsestory\u0009")("Parse story or stories to suite(s), followed by stories path or specific '.feature' file path\n");
    ...
    wrapBlock: (type, text, loopCount) => {
        if ( planConfig.get().consoleOutput === 'step' ) {
            if ( "given" === type) {
