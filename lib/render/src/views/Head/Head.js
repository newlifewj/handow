import { React, cx, eventBus } from '../../Depends';
    ...
    render() {
        return (
            <div className={cx(style.head)}>
                ...
                <div className={cx(style.logo)}  onClick={ this.backHome }
                    style={{ marginLeft: this.props.width == 'xs' ? '5px' : '30px', cursor: "pointer" }}></div>

                <div id='logo-head' className={style.title}>
                    <span>{window._projectTitle} - {record.getPlanTitle()}</span>
