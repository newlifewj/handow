...
const _watchResp = async (resp, page, xhr) => {
    ...
    if ( `${ resp.url() }`.includes(`${ xhr.url }`)
        ...
        const reqUrl = `${ resp.url() }`.includes(`${ xhr.url }`) ? `${ resp.url() }` : "(Unrecognized path)";
        const reqMethod = methods.includes(`${req.method().toLowerCase()}`)
                        && `${req.method().toLowerCase()}` === xhr.method.toLowerCase()
                        ? `${req.method()}` : "(Unrecognized method)";
        page['xhr'] = {
            request: {
                path: reqUrl
            },
 ...
 * @throws - Throw timeout broken error if element not found in 1 second
 */
 When("I click {xpath} and watch {xhr}", async (xpath, xhr) => {
...
/**
 * Mouse click an element specified by selector if it is visible now, so this action always pass
 * @param {string} selector - A valid css-selector or Handow-probe
 * @throws - Throw timeout broken error if element not found in 1 second
 */
 When("I click {selector} if it is visible", async (selector) => {
    const _sel =  await page.waitForSelector(selector, { visible: true, timeout: 1000 });
    if ( _sel ) {
        const elements = await page.$$(selector);
        // pptr tricky, re-evalated element and click will work well. Click element immediately is not stable.
        await page.evaluate( el => el.click(), elements[0] );
...
/**
 * Mouse click an element specified by xpath if it is visible now, so this action always pass
 * @param {string} xpath - A valid css-selector or Handow-probe
 * @throws - Throw timeout broken error if element not found in 1 second
 */
 When("I click {xpath} if it is visible", async (xpath) => {
    const _xp =  await page.waitForSelector(xpath, { visible: true, timeout: 1000 });
    if ( _xp ) {
        const elements = await page.$x(xpath);
        // pptr tricky, re-evalated element and click will work well. Click element immediately is not stable.
        await page.evaluate( el => el.click(), elements[0] );
...
When("I click {selector} and choose the files {paths}", async (selector, paths) => {
    ...
    // const [fileChooser] - Destructuring assignment. The Promise.all([..., ...]) return an array of promises ordered with the inputs
    // Promise.all([...]) - Resolve promises after all resolved or any one rejected or exception throwed.
    const [fileChooser] = await Promise.all([
        page.waitForFileChooser(),  // Wait the fileChooser handle, here it is returned after the file selected because they are warapped in promise.all
 ...
 * Note: Similar with "I send request {xhr}"
 */
 ...
 * @throws - Throw timeout error if the elements not available in configured time
 */
 When("I wait {selector} presented", async (selector) => {
    await page.waitForSelector(selector, { visible: true, timeout: config.elementAppearTimeout });
});
 ...
 * @throws - Throw timeout error if the elements not available in configured time
 */
When("I wait {xpath} presented", async (xpath) => {
    await page.waitForXPath(xpath, { visible: true, timeout: config.elementAppearTimeout });
});
...
When("I wait {selector} is disappeared", async (selector) => {
    ...
    // ----------
    await page.waitForSelector(selector, { state: 'hidden', timeout: config.elementAppearTimeout });
});
 ...
 * @throws - Throw timeout error if the elements are still displayed in configured time
 */
 When("I wait {selector} absented", async (selector) => {
    await page.waitForSelector(selector, { state: 'hidden', timeout: config.elementAppearTimeout });
});
 ...
 */
 When("I wait {selector} disappeared", async (selector) => {
     try {
        // Test element visible or not at this moment
        await page.waitForSelector(selector, { visible: true, timeout: 500 });

        // Continue if the element is visible
        try {
            // If element is visible, then wait it to be hidden
            await page.waitForSelector(selector, { hidden: true, timeout: config.elementAbsentTimeout });
        } catch (_err) {
            throw new TypeError("_Elemnt_Visible_");
        }
     } catch (err) {
        if (`${err}`.includes("_Elemnt_Visible_")) {
            throw new TypeError(`Element didn't disappear in (${config.elementAbsentTimeout}ms) watching period`);
        }
     }
});
 ...
 * @throws - Throw timeout error if the elements are still displayed in configured time
 */
When("I wait {xpath} disappeared", async (xpath) => {
    try {
        // Test element visible or not at this moment
        await page.waitForXPath(xpath, { visible: true, timeout: 500 });

        // Continue if the element is visible
        try {
            // If element is visible, then wait it to be hidden
            await page.waitForXPath(xpath, { hidden: true, timeout: config.elementAbsentTimeout });
        } catch (_err) {
            throw new TypeError("_Elemnt_Visible_");
        }
     } catch (err) {
