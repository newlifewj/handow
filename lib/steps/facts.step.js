 ...
 */
/*
Then("I cannot see {selector} is displayed", async (selector) => {
    try {
 ...
 * @throws - Expectation and custom timeout error if, finally, the elements exist and visible
 */
 Then("I cannot see {selector} is visible", async (selector) => {

    const element = await page.waitForSelector(selector, { hidden: true, timeout: config.elementAbsentTimeout });
    if ( element === null ) {
        return true;
    } else {
        throw new TypeError(`Timeout (${config.elementAbsentTimeout}ms) for waiting element disappeared`);
    }
...
Then("I cannot see {xpath} is visible", async (xpath) => {

    const element = await page.waitForXPath(xpath, { hidden: true, timeout: config.elementAbsentTimeout });
    if ( element === null ) {
        return true;
    } else {
        throw new TypeError(`Timeout (${config.elementAbsentTimeout}ms) for waiting element disappeared`);
