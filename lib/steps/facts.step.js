 ...
 * @throws - Custom exception for verification error or timeout 
 */
 ...
 * @throws - Custom exception for timeout 
 */
...
/**
 * Verify if the visible content (showing on screen) of specified element CONTAINS given text
 * @param {string} selector - Valid css-selector or Handow-probe
 ...
 * @throws - Unexpected result or defaultTimeout
 */
 ...
 * @throws - Unexpected result or timeout
 */
...
/**
 * Verify the specified input element with given value
...
Then("I verify the property {name} of {selector} equals {value}", async (name, selector, value) => {
    ...
    const attrValue = await page.evaluate( ([el, attr]) => el.getAttribute(attr), [elements[0], name] );    // Pass 2 agrs to page function
...
/**
 * Verify a new tab opened, e.g. href _target-blank (by searching new tabs/windows and check the URL is matching or not)
...
/**
 * A new window is opened (It is a popup view), e.g. triggered by window.open() API, then verify the URL of the new window toBe expected
...
/**
 * Verify cookie items exist in current browser context by checking cookie-names, e.g. "JSSESSION,APIID,..."
 ...
 * @Note - This step should cooperate with relevant When steps sending XHR or watching specified XHR
 */
...
/**
 * Verify if each of the specified container elements includes at least one content element.
 ...
 Then("I can see all {xpathContainer} including {xpathContent}", async (xpathContainer, xpathContent) => {
    ...
    if (containers && containers.length > 0) {
        try {
            ...
            await Promise.all( Object.keys(containers).map(
 ...
 * @throws - Custom errors
 */
Then("I can find {file} in {folder}", (file, folder) => {
